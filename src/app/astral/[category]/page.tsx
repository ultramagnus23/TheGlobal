...
import { astralCategories } from "@/content/divisions";
import { getProductsByCategory } from "@/content/products";
import { canonical } from "@/lib/seo";

export function generateStaticParams() {
  return astralCategories.map((c) => ({ category: c.slug }));
}
...
export async function generateMetadata({
  ...
  const found = astralCategories.find((c) => c.slug === category);
  if (!found) return {};
  return {
    title: `${found.name} — Astral, The Global`,
    description: found.blurb,
    alternates: { canonical: canonical(`/astral/${category}`) },
  };
}

export default async function AstralCategoryPage({
  params,
}: {
  params: Promise<{ category: string }>;
}) {
